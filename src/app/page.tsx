    ...
    <div className="glow">
        ...
        <div className="grid items-start gap-12 lg:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <dl className="mt-12 grid grid-cols-2 gap-3 lg:grid-cols-4">
              <Stat label="Paid out" value={fmtUsd(stats.paid_cents)} />
              <Stat label="Earned" value={fmtUsd(stats.earned_cents)} />
              <Stat label="Tokens" value={String(stats.tokens)} />
              <Stat label="Creators" value={String(stats.creators)} />
            </dl>
          </div>
          <LiveFeed initial={{ demo, feed, stats }} compact />
        </div>
      </section>
          ...
          {[
            {
              n: "01",
              t: "Launch",
              d: "Fill in a name, ticker and image, and pick the TikTok @handle that should get paid. TikPad creates the token on pump.fun with its treasury as the on-chain creator.",
            },
            {
              n: "02",
              t: "Fees accrue",
              d: "Every trade on pump.fun pays a creator fee. Because the treasury is the creator, those fees land with TikPad and are attributed to the token that produced them.",
            },
            {
              n: "03",
              t: "Creator gets paid",
              d: "80% of every claim is credited to the TikTok handle. When the balance crosses $5, then $10, $20, $50 and so on, it is sent to the wallet the creator linked by signing in with TikTok.",
            },
          ].map((s) => (
            <div key={s.n} className="card p-6">
              <div className="num text-xs text-dim">{s.n}</div>
      ...
      {top.length > 0 && (
        <section className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
          <div className="flex items-end justify-between">
            ...
            <Link href="/claim" className="text-sm text-muted hover:text-fg">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <div className="card flex flex-col items-start justify-between gap-6 p-8 sm:flex-row sm:items-center">
          ...
          <Link href="/claim" className="btn btn-accent">
            Claim your earnings
          </Link>
        </div>
      </section>
